...
describe('our first suite', () => {
  ...
  it('assert property', () => {
    ...
    cy.contains('nb-card', 'Common Datepicker').find('input').then(input => {
      ...
      cy.wrap(input).invoke('prop', 'value').should('contain', 'Jun 24, 2021')
    })
  })
  it('radio button', () => {
  ...
  it('web-tables', () => {
    ...
    // 2
    // from example scenario: add new record
    cy.get('thead').find('.nb-plus').click()
    cy.get('thead').find('tr').eq(2).then(tableRow => {
      cy.wrap(tableRow).find('[placeholder="First Name"]').type('Vlad')
      cy.wrap(tableRow).find('[placeholder="Last Name"]').type('Mykytenko')
      cy.wrap(tableRow).find('.nb-checkmark').click()

      cy.get("tbody tr").first().find('td').then(tableColumns => {
        cy.wrap(tableColumns).eq(2).should('contain', 'Vlad')
        cy.wrap(tableColumns).eq(3).should('contain', 'Mykytenko')
      })
    })

    //3 filtering scenario
